  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (vehicle) => vehicle.make.toLowerCase().includes(term) || vehicle.model.toLowerCase().includes(term),
      )
    }
    ...
    // Apply year filter
    if (filterYear !== "all") {
      result = result.filter((vehicle) => vehicle.year.toString() === filterYear)
    }
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted">
                  <tr>
                    ...
                    <th className="py-3 px-4 text-left font-medium">Last Service</th>
                ...
                <tbody className="divide-y">
                  {filteredVehicles.slice(0, visibleVehicles).map((vehicle) => (
                    <tr key={vehicle.id} className="hover:bg-muted/50">
                      ...
                      <td className="py-3 px-4">{vehicle.lastService}</td>
                      <td className="py-3 px-4">
                        <Button variant="ghost" size="sm" onClick={() => router.push(`/vehicle-details/${vehicle.id}`)}>
                          Details
                        </Button>
                      </td>
                    </tr>
